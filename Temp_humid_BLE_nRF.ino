...
BLEUnsignedCharCharacteristic Gas_char("2101",BLERead | BLENotify);
...
void setup() {
  ...
  Environmental_Sensing.addCharacteristic(Gas_char); 
...
void updateReadings() {
   ...
   int gas_level=map(gas_value,0,1023,0,100);
   
   if(gas_level !=prev_gaslevel){
   
   Serial.print("Gas sensor level % is: ");
   Serial.println(gas_level);
   Gas_char.writeValue(gas_level);
   prev_gaslevel=gas_level;
   }
